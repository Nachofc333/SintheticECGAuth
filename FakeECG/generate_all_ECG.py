...
cvae.load_state_dict(torch.load(f"FakeECG/{modelname}.pth", map_location=device))
cvae.eval()
...
if modelname == "model500":
    ...
    # Cargar X_train desde el archivo npy
    X_train = np.load(f"FakeECG/X_train{modelname}.npy")
    # Cargar y_train desde el archivo npy
    y_train = np.load(f"FakeECG/y_train{modelname}.npy")  
